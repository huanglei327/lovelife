...
export default {
  ...
  //     default: 1
  //   },
  //   imgUrl: 'http://slls-img.dsunyun.com/90151148507.jpg'
  // },
  data() {
    return {
      imgHL: ""
  ...
  watch: {
    file(val) {
      this.imgData.file = val;
      this.change(val);
    }
  },
  computed: {
    file() {
      return this.imgData.file;
    }
  },
  mounted() {},
  methods: {
    ...
    initilize(opt) {
      ...
      this.cropper = new Cropper(this.preview, {
        ...
        zoomable: true, //是否允许放大图像。
        zoomOnWheel: false, //是否可以通过拖动触摸来放大图像。
        center: false,
    ...
    clip(e, opt) {
      ...
      this.initilize(opt);
      //获取图片文件资源
      //this.picValue = files.file;
      //alert(JSON.stringify(this.picValue.size))
